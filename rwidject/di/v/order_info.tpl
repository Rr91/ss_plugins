<?php echo $header; ?><?php echo $column_left; ?>
   ...
   <?
     /*   if(isset($account_custom_fields[2]['value'])){


            $aa = str_replace(" ","",$account_custom_fields[2]['value']);


        $aa =  unserialize($aa);
 print_r($aa);
           foreach(unserialize($account_custom_fields[2]['value']) as $item){
                print_r($item);
            }
        }*/
   ?>
  ...
  <div class="page-header">
    <div class="container-fluid">
      <div class="pull-right">
        <? if(!$parent_id){?><button type="button" id="button-order_copy" class="btn btn-default" data-original-title="Копировать"><i class="fa fa-copy"></i></button><? } ?>
        <? if($count==2){ ?>
          <fieldset>
              <a href="<?php echo $printinvoice1; ?>" target="_blank" data-toggle="tooltip" title="<?php echo $button_printinvoice; ?>" class="btn btn-info"><i class="fa fa-print"></i></a>
              <a href="<?php echo $printinvoice2; ?>" target="_blank" data-toggle="tooltip" title="<?php echo $button_printinvoice; ?>" class="btn btn-info"><i class="fa fa-print"></i></a>
          </fieldset>
          <fieldset>
              <a href="<?php echo $printorder1; ?>" target="_blank" data-toggle="tooltip" title="<?php echo $button_printorder; ?>" class="btn btn-info"><i class="fa fa-print"></i></a>
              <a href="<?php echo $printorder2; ?>" target="_blank" data-toggle="tooltip" title="<?php echo $button_printorder; ?>" class="btn btn-info"><i class="fa fa-print"></i></a>
          </fieldset>
        <? }else{ ?>
          <a href="<?php echo $printinvoice; ?>" target="_blank" data-toggle="tooltip" title="<?php echo $button_printinvoice; ?>" class="btn btn-info"><i class="fa fa-print"></i></a>
          <a href="<?php echo $printorder; ?>" target="_blank" data-toggle="tooltip" title="<?php echo $button_printorder; ?>" class="btn btn-info"><i class="fa fa-print"></i></a>
        <? } ?>
        ...
        <div class="panel panel-default">
          ...
          <table class="table">
            <tbody>
              ...
              <?php } ?>
              <tr>
                <td><button data-toggle="tooltip" title="Суммарный вес товаров" class="btn btn-info btn-xs"><i class="fa fa-balance-scale fa-fw"></i></button></td>
    ...
    <div class="panel panel-default">
      ...
      <div class="panel-body">
        ...
        <table class="table table-bordered">
          <thead>
            <tr><td class="text-left">Дочерние заказы</td></tr>
          </thead>
          <tbody>
            <?php foreach ($children as $href=>$child) { ?>
              <tr><td class="text-left"><a href="<?=$href?>" target="_blank">#<?=$child['order_id']?>.<?=$child['parent_id']?>.<?=$child['num']?></a></td></tr> 
            <?}?>
          </tbody>
        </table>
        <?}else if($parent_href){?>
          <table class="table table-bordered">
          <thead>
            <tr><td class="text-left">Родительский заказ</td></tr>
          </thead>
          <tbody>
            <tr><td><a href="<?=$parent_href?>" target="_blank">#<?=$parent_id?></a></td></tr>
          </tbody>
        </table>
        <?}?>
        <table class="table table-bordered">
            ...
            <fieldset>
              ...
              <form class="form-horizontal">
                ...
                <div class="form-group">
                  ...
                  <div class="form-group">
                      <label class="col-sm-2 control-label" for="input-notify">Тип сообщения: </label>
                      <div class="col-sm-10">
                          <select name="comment_header" id="comment_header">
                              <option value="fio">адрес на английском</option>
                              <option value="track">Трек-номер</option>
...
$(document).delegate('#button-order_copy', 'click', function(e) {
	$.ajax({
		url: 'index.php?route=module/divide&token=<?php echo $token; ?>&order_id=<?php echo $order_id; ?>',
		dataType: 'json',
		beforeSend: function() {
			$('#button-order_copy').button('loading');
		},
		complete: function() {
			$('#button-order_copy').button('reset');
		},
		success: function(json) {
...
$('#button-history').on('click', function() {
	...
	$.ajax({
		...
		type: 'post',
		dataType: 'json',
		data: 'order_status_id=' + encodeURIComponent($('select[name=\'order_status_id\']').val()) + '&notify=' + ($('input[name=\'notify\']').prop('checked') ? 1 : 0) + '&override=' + ($('input[name=\'override\']').prop('checked') ? 1 : 0) + '&append=' + ($('input[name=\'append\']').prop('checked') ? 1 : 0) + '&comment=' + encodeURIComponent($('textarea[name=\'comment\']').val()) + '&comment_head=' + $('select[name=\'comment_header\']').val(),
		beforeSend: function() {
